"""MaxRL return-EMA run: reweight advantages by inverse percentile of per-env return EMA."""
...
def main() -> None:
    ...
    args.custom_experiment_name = f"maxrl_return_ema_{args.custom_experiment_name}"
    args.wandb_tags = list(args.wandb_tags) + ["maxrl_return_ema"]
    ...
    cmd_parts = [
        ...
        # === MaxRL: RETURN-EMA reweighting ===
        "train.params.config.advantage_reweight_mode=return_ema",
        "train.params.config.advantage_reweight_eps=0.1",
        "train.params.config.advantage_reweight_ema_decay=0.99",
        # === Wandb ===
