...
class RosSnapshot:
    ...
    def make_copy_with_defaults(self) -> RosSnapshot:
        ...
        if self.object_pose_2 is None:
            warn_every("object_pose_2 is None", n_seconds=1.0)
        ...
        if self.goal_object_pose_2 is None:
            warn_every("goal_object_pose_2 is None", n_seconds=1.0)
...
class VisualizationNode:
    def __init__(self, object_name: str, object_name_2: str):
        # ROS setup
        rospy.init_node("visualization_node_2_objects")
        ...
        # Initialize Viser
        self.initialize_viser(object_name=object_name, object_name_2=object_name_2)
    ...
    def initialize_ros_subscribers(self):
        ...
        self.goal_object_pose_2_sub = rospy.Subscriber(
            "/robot_frame/goal_object_pose_2",
            Pose,
            self.goal_object_pose_2_callback,
            queue_size=1,
        )

    def initialize_viser(self, object_name: str, object_name_2: str):
        ...
        assert object_urdf_2.exists(), f"object_urdf_2 does not exist: {object_urdf_2}"
        ...
        self.object_2_viser = SERVER.scene.add_frame(
            "/object_2",
            position=FAR_AWAY_OBJECT_POSITION + np.array([0.0, 0.2, 0.0]),
            wxyz=(1, 0, 0, 0),
            show_axes=True,
            axes_length=AXES_LENGTH,
            axes_radius=AXES_RADIUS,
        )
        self.object_2_urdf_viser = ViserUrdf(
            SERVER, object_urdf_2, root_node_name="/object_2"
        )
        ...
        self.goal_object_2_viser = SERVER.scene.add_frame(
            "/goal_object_2",
            position=FAR_AWAY_OBJECT_POSITION + np.array([0.2, 0.4, 0.2]),
            wxyz=(1, 0, 0, 0),
            show_axes=True,
            axes_length=AXES_LENGTH,
            axes_radius=AXES_RADIUS,
        )
        self.goal_object_2_urdf_viser = ViserUrdf(
            SERVER,
            goal_object_urdf_2,
            root_node_name="/goal_object_2",
            mesh_color_override=GREEN_RGBA,
        )
    ...
    def object_pose_2_callback(self, msg: PoseStamped):
        """ "Callback to update the second current object pose."""
        msg = msg.pose
        xyz = np.array([msg.position.x, msg.position.y, msg.position.z])
        quat_xyzw = np.array(
            [
    ...
    def goal_object_pose_2_callback(self, msg: Pose):
        """ "Callback to update the second goal object pose."""
        xyz = np.array([msg.position.x, msg.position.y, msg.position.z])
        quat_xyzw = np.array(
            [
                msg.orientation.x,
    ...
    def update_viser(self):
        ...
        # Update the second object pose
        # Object pose is in camera frame = C frame
        # We want it in world frame = robot frame = R frame
        T_R_O_2 = object_pose_2
        T_W_O_2 = T_W_R @ T_R_O_2
        object_2_pos = T_W_O_2[:3, 3]
        object_2_quat_xyzw = R.from_matrix(T_W_O_2[:3, :3]).as_quat()
        self.object_2_viser.position = object_2_pos
        self.object_2_viser.wxyz = object_2_quat_xyzw[[3, 0, 1, 2]]
        ...
        # Update the second goal object pose
        # Goal object pose is in camera frame = C frame
        # We want it in world frame = robot frame = R frame
        T_R_G_2 = goal_object_pose_2
        T_W_G_2 = T_W_R @ T_R_G_2
        goal_object_2_pos = T_W_G_2[:3, 3]
        goal_object_2_quat_xyzw = R.from_matrix(T_W_G_2[:3, :3]).as_quat()
        self.goal_object_2_viser.position = goal_object_2_pos
        self.goal_object_2_viser.wxyz = goal_object_2_quat_xyzw[[3, 0, 1, 2]]
...
class VisualizationNodeArgs:
    ...
    object_name_2: str = "claw_hammer"
    f"""The name of the second object to visualize. Options: {", ".join(ALL_OBJECT_NAMES)}"""
...
def main():
    ...
    try:
        ...
        node = VisualizationNode(
            ...
            object_name_2=args.object_name_2,
        )
        node.run()
